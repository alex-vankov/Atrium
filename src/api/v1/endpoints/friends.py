...
def get_friend_requests(user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    """
    return view_friend_requests(db, user)


@router.post("/requests")
def send_friend_request(receiver_id: uuid.UUID, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    """
    Send a friend request.
    """
    return create_friend_request(db, user, receiver_id)
...
def respond_friend_request(friend_request_id: uuid.UUID,
    ...
    """
    return open_friend_request(db, user, friend_request_id, action)
